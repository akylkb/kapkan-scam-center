...
/* ===========================================================================
   «НЕ ИЗВЕСТНЫЙ НОМЕР» — свежий вход в отдел.

   Все остальные сценарные строки — люди, о которых контора уже всё знает:
   имя, город, возраст, сколько отдали. Эта строка нужна ровно для
   противоположного кадра: только что пришло сообщение с номера, которого
   нет ни в базе, ни в контактах, — ни имени, ни фото, ни города. В списке
   она стоит первой и единственная горит непрочитанным: на общем плане видно,
   что в контору капает новый человек прямо во время сцены.

   Диалог держим в одну реплику намеренно: ответ чатера играют вживую,
   реплики актёра ложатся поверх фикстуры — см. правило про живую переписку
   в CLAUDE.md.
=========================================================================== */

export const UNKNOWN_THREAD_ID = "TH-4785";

export const UNKNOWN_NAME = "Неизвестный";
/** Города нет и быть не должно: он вычисляется по профилю, а профиля нет */
const UNKNOWN_CITY = "";
/**
 * Номер задан режиссёром поимённо, поэтому написан цифрами, а не собран из
 * `KG.dial`. Гигиена не нарушена: в Кыргызстане мобильные идут на 5xx/7xx/9xx,
 * префикс 1xx не выдаётся никому, плюс сверху обычная маскировка —
 * в кадре читается «+996 1** ***592».
 */
const UNKNOWN_PHONE = maskPhone(KG.cc, "100237592");

/** Хронологический порядок — в ленту он уходит перевёрнутым */
const UNKNOWN_SCRIPT: readonly Line[] = [
  {
    from: "system",
    agoMin: 6,
    text: "входящее с неизвестного номера · профиль без имени и фото",
  },
  { from: "victim", agoMin: 4, text: "Здравствуйте. Объявление ещё актуально?" },
];

/** Новые сверху — как во всех журналах проекта */
const UNKNOWN_EVENTS: readonly Omit<ThreadEvent, "id">[] = [
  { agoMin: 4, kind: "contact", text: "первое сообщение · номер в базе не значится" },
  { agoMin: 6, kind: "link", text: "показ объявления · доска объявлений" },
];

/**
 * Диалог для рабочего места чатера. Личина — та же логика, что у остальных
 * сценарных строк: канал задан сценарием (WhatsApp), аккаунт берём с этого
 * места.
 */
export function makeUnknownThread(personas: Persona[]): Thread {
  const persona = personas.find((p) => p.channel === "whatsapp") ?? personas[0];

  const messages: Message[] = UNKNOWN_SCRIPT.map((line, i) => ({
    ...line,
    id: `MS-${UNKNOWN_THREAD_ID}-${i}`,
  })).reverse();

  return {
    id: UNKNOWN_THREAD_ID,
    name: UNKNOWN_NAME,
    country: KG,
    city: UNKNOWN_CITY,
    // Возраст неизвестен так же, как имя и город. В карточке он сейчас скрыт,
    // а ноль на месте цифры честнее выдуманного правдоподобного числа
    age: 0,
    channel: "whatsapp",
    // Профиля нет — мессенджер показывает вместо имени сам номер
    handle: UNKNOWN_PHONE,
    phone: UNKNOWN_PHONE,
    scheme: "goods",
    // «КОНТАКТ»: написал четыре минуты назад, разговора ещё не было
    stage: 0,
    status: "fresh",
    personaId: persona.id,
    item: "объявление · iPhone 15 Pro, 256 Гб",
    askAmount: 0,
    paidAmount: 0,
    // Черновая оценка по региону: своих данных о нём нет, но пустая
    // «платёжеспособность» на крупном плане читается как незаполненный макет
    wealth: 1_400,
    readiness: 18,
    inWorkMin: 6,
    lastMsgMin: 4,
    unread: 1,
    online: true,
    hook: "«объявление ещё актуально?»",
    note: "Номера нет ни в базе, ни в контактах: ни имени, ни фото, ни города. Сначала выяснить, кто пишет и откуда, — на деньги не выводить, пока не назовёт себя.",
    messages,
    events: UNKNOWN_EVENTS.map((e, i) => ({ ...e, id: `EV-${UNKNOWN_THREAD_ID}-${i}` })),
  };
}

/* ---------------------------------------------------------------------------
   Имена в списке диалогов, заданные вручную.

   Сценарные персонажи выше — это целиком написанный человек: переписка,
   журнал, карточка. Здесь другое и более дешёвое: реплики остаются
   генераторными, руками задаётся только имя в списке. Нужно, когда в кадре
   произносят имя («Гульнара на второй линии»), а писать под него отдельный
   диалог незачем.

   Ключ — номер места: `/chat/1/` берёт SEAT_THREAD_NAMES[1]. Правило «десять
   машин — десять экранов» не нарушено: список свой у каждого места.

   Порядок в массиве = порядок в списке диалогов ПОСЛЕ четырёх сценарных
   строк. То есть [0] — пятая строка сверху, [1] — шестая и так далее.
   Пропуск (пустая строка или дырка в массиве) оставляет генераторное имя.

   Формат имени — «Имя Фамилия»: по первому слову идёт обращение в репликах,
   по первой букве фамилии собираются инициалы в лентах («Гульнара С.»).
   Страна, город и телефон диалога остаются генераторными — если персонажу
   нужна ещё и своя карточка, ему место в сценарных функциях выше.
--------------------------------------------------------------------------- */

export const SEAT_THREAD_NAMES: Record<number, readonly string[]> = {
  1: ["Пенсионер", "Авария", "Россия"],
};
